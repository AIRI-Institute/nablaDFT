...
def parse_command_line_arguments():
    ...
    args_hyperparams.add_argument("--activation", metavar='STR', type=str, default='swish', choices=['ssp', 'swish'],
                                  help="which activation function to use (shifted softplus (ssp) or swish))")
    args_hyperparams.add_argument("--order", metavar='INT', type=int, default=2,
                                  help="angular order of the feature vectors")
    args_hyperparams.add_argument("--num_features", metavar='INT', type=int, default=32,
                                  help="dimensionality of feature vectors")
    args_hyperparams.add_argument("--num_basis_functions", metavar='INT', type=int, default=32,
    ...
    args_hyperparams.add_argument("--num_residual_full_ii", metavar='INT', type=int, default=1,
                                  help="number of residual blocks for predicting diagonal blocks (full hamiltonian)")
    args_hyperparams.add_argument("--num_residual_full_ij", metavar='INT', type=int, default=1,
                                  help="number of residual blocks for predicting off-diagonal blocks (full hamiltonian)")
    args_hyperparams.add_argument("--num_residual_core_ii", metavar='INT', type=int, default=1,
                                  help="number of residual blocks for predicting diagonal blocks (core hamiltonian)")
    args_hyperparams.add_argument("--num_residual_core_ij", metavar='INT', type=int, default=1,
                                  help="number of residual blocks for predicting off-diagonal blocks (core hamiltonian)")
    args_hyperparams.add_argument("--num_residual_over_ij", metavar='INT', type=int, default=1,
    ...
    args_training.add_argument("--valid_batch_size", metavar='INT', type=int, default=1,
                               help="batch size for validation")
    args_training.add_argument("--num_workers", metavar='INT', type=int, default=0,
    ...
    args_training.add_argument("--learning_rate", metavar='FLOAT', type=float, default=1e-3,
    ...
    args_training.add_argument("--full_hamiltonian_weight", metavar='FLOAT', type=float, default=1.0,
                               help="weight of the full hamiltonian in the loss function")
    args_training.add_argument("--core_hamiltonian_weight", metavar='FLOAT', type=float, default=1.0,
                               help="weight of the core hamiltonian in the loss function")
    args_training.add_argument("--overlap_matrix_weight", metavar='FLOAT', type=float, default=1.0,
                               help="weight of the overlap matrix in the loss function")
    args_training.add_argument("--energy_weight", metavar='FLOAT', type=float, default=1.0,
                               help="weight of the energy in the loss function")
    args_training.add_argument("--forces_weight", metavar='FLOAT', type=float, default=1.0,
                               help="weight of the forces in the loss function")
    args_training.add_argument("--predict_energy", metavar='True|False', type=bool, default=False,
    ...
    args_training.add_argument("--weight_decay", metavar='FLOAT', type=float, default=0.0,
                               help="regularization term for weights")
    args_training.add_argument("--use_gpu", metavar='True|False', type=str2bool, default=True, choices=[True, False],
                               help="use GPU(s) for training (if available)")
    args_training.add_argument("--nproc_per_node", metavar='INT', type=int, default=1,
                               help="use GPU(s) for training (if available)")
    args_training.add_argument("--nnodes", metavar='INT', type=int, default=1,
                               help="use GPU(s) for training (if available)")
    args_training.add_argument("--local_rank", metavar='INT', type=int, default=0,
                               help="use GPU(s) for training (if available)")
    args_training.add_argument("--node_rank", metavar='INT', type=int, default=0,
                               help="use GPU(s) for training (if available)")

    # arguments for logging and checkpoints
    args_logging = parser.add_argument_group("logging and checkpoints")
    args_logging.add_argument("--write_parameter_summaries", metavar='True|False', type=str2bool, default=False,
                              choices=[True, False], help="write summaries for parameters")
    ...
    args_logging.add_argument("--summary_interval", metavar='INT', type=int, default=1,
                              help="log summaries after every INT steps")
    args_logging.add_argument("--checkpoint_interval", metavar='INT', type=int, default=1,
