...
class HamiltonianDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, filepath, max_batch_orbitals=1200, max_batch_atoms=150, max_squares=4802, subset=None, dtype=torch.float32):
    ...
    def collate_fn(self, batch, return_filtered=False):
        ...
        for batch_num, data in enumerate(all_data):
            ...
            for z in Z_:
                local_orbitals.append(tuple((int(z),int(l)) for l in self.database.get_orbitals(z)))
            ...
            if orbitals_number + local_orbitals_number > self.max_batch_orbitals or\
               len(local_orbitals) + len(orbitals) > self.max_batch_atoms or \
               sum_squares + len(local_orbitals) ** 2 > self.max_squares:
                break
